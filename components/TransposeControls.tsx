...
export function TransposeControls({ semitones, setSemitones, preference, setPreference, className }: TransposeControlsProps) {
    ...
    return (
        <div className={cn("bg-white p-8 rounded-[3rem] shadow-2xl shadow-indigo-200/50 ring-1 ring-black/5", className)}>
            <div className="flex flex-col gap-6">
                ...
                {/* Central Control Area (Flanking Layout) */}
                <div className="flex items-center justify-center gap-8 py-2">
                    ...
                    <button
                        ...
                        className="h-20 w-20 flex-none flex items-center justify-center rounded-full bg-slate-50 border-4 border-white shadow-lg shadow-slate-200 hover:bg-slate-100 hover:scale-105 active:scale-95 transition-all disabled:opacity-50 disabled:active:scale-100"
                        aria-label="Lower key"
                    >
                        <Minus size={24} strokeWidth={3} className="text-slate-600" />
                    </button>

                    {/* Display */}
                    <div className="flex flex-col items-center min-w-[120px]">
                        <span className={cn(
                            "text-6xl font-black tracking-tighter tabular-nums leading-none filter drop-shadow-sm",
                            semitones === 0 ? "text-slate-300" :
                                semitones > 0 ? "text-indigo-600" : "text-emerald-500"
                        )}>
                    ...
                    <button
                        onClick={() => setSemitones(semitones + 1)}
                        disabled={semitones >= 12}
                        className="h-20 w-20 flex-none flex items-center justify-center rounded-full bg-indigo-600 border-4 border-indigo-100 shadow-xl shadow-indigo-600/30 hover:bg-indigo-500 hover:scale-105 active:scale-95 transition-all disabled:opacity-50 disabled:active:scale-100"
                    ...
                    {[
                        ...
                        return (
                            <button
                                ...
                                className={cn(
                                    "flex-1 px-4 py-3 rounded-[1.5rem] text-xs font-bold transition-all transform",
                                    isActive
